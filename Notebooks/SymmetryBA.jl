...
# ## Antipode
# Since we know that the inverse element in the Butcher group is defined as $a^{-1}=a \circ S$, where S is the so-called antipode, I need to define this antipode first.
...
function antipode(t::RootedTree)
    ...
    # Collecting the subtrees that remain after removing the trees
...
# ### Implicit Midpoint
# The implicit midpoint method is symmetric. Thus, the symmetric order will be always the same as the order I choose for the B-series.
