...
function plot_firstode()
    ...
    norm_error1[j] = norm(y) - norm(res.zero)
    j = j + 1
    end
    Plot1 = scatter(dt_values, norm_error1, title = "Fehler in der Norm für die erste ODE", xlabel = L"dt", ylabel = L"|u^0| - |u^1|", label="error for spepsize")
    savefig("Plot1.png")
    return Plot1   
end
...
function plot_secondode()
    ...
    norm_error2[j] = norm(y) - norm(res.zero)
    j = j + 1
    end
    Plot2 = scatter(dt_values, norm_error2, title = "Fehler in der Norm für die zweite ODE", xlabel = L"dt", ylabel = L"|u^0| - |u^1|", label="error for spepsize")
    savefig("Plot2.png")
    return Plot2
end
